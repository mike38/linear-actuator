	...
	difference()
	{
		union()
		{
			if (open_frame == true)
			{
				...
				minkowski()
				{
					cube(planetary_gearbox_size-[1, 1, 1], center=true);
			...
			else
			{
				...
				minkowski()
				{
					cube(planetary_frame_size-[1, 1, 1], center=true);
					cylinder(h=1, r=2, $fn=12, center=true);
				}
			}
...
module sun_gear()
{
	difference()
	{
		union()
		{
			...
			cylinder(h=sun_gear_hub_height, r=sun_gear_hub_radius, $fn=40, center=true);
		...
		union()
		{
			...
			cylinder(h=sun_gear_height+.1, r=sun_bore_radius, $fn=42, center=true);

			translate(sun_gear_hub_center+[0, sun_gear_hub_radius/2+1, 0])
			rotate([90, 0, 0])
			{
				...
				translate([0, -3, 0])
				cube([m3_nut_diameter*cos(30), 6, m3_nut_thickness+.5], center=true);
			}
		}
	}
}
...
module cover(print_orientation=1)
{
	...
	difference()
	{
		...
		minkowski()
		{
			cube(cover_size-[2, 2, 1], center=true);
			cylinder(h=1, r=2, $fn=12, center=true);
		}
